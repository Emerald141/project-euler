...
# any instruction not concerned with movement will be ignored and the player
...
# THEORY:
# Instead of a vigorous proof, this program runs a Monte Carlo simulation.
# After enough rolls, the three squares with the highest experimental
# probabilities are all but guaranteed to be the three squares with the
# highest theoretical probabilities.
...
def solve(simulation_count = 1000000):
    start = time()
    chance = [7, 22, 36]
    community_chest = [2, 17, 33]
    rolls = [2, 3, 4, 5, 3, 4, 5, 6, 4, 5, 6, 7, 5, 6, 7, 8]  # Equally likely
    landings = [0 for tile in range(40)]
    double_streak = 0
    position = 0
    for simulation in range(simulation_count):
        roll = randint(0, len(rolls)-1)
        if roll % 5 == 0:  # Rolled a double
            double_streak += 1
            if double_streak == 3:
                position =  10  # JAIL
                double_streak = 0
            ...
            position += rolls[roll]
            position %= 40
        if position in chance or position in community_chest:
            draw = randint(1, 16)
            if position in chance:
                if draw == 1: position = 0  # GO
                elif draw == 2: position = 10  # JAIL
                if draw == 3: position = 11  # C1
                elif draw == 4: position = 24  # E3
                elif draw == 5: position = 39  # H2
                elif draw == 6: position = 5  # R1
                    ...
                    if position == 7 or position == 36:
                        ...
                        position = 28
                elif draw == 10: position -= 3  # Go back 3 spaces
            if position in community_chest:
                if draw == 1: position = 0  # GO
                elif draw == 2: position = 10  # JAIL
        # end of chance and community chest loop
        if position == 30:  # G2J
            position = 10  # JAIL
        landings[position] += 1
    # end of simulation loop
    # Task is now to find indices of three largest elements in landings
    first_count, second_count, third_count = 0, 0, 0
    first_position, second_position, third_position = 0, 0, 0
    for position in range(len(landings)):
        if landings[position] > first_count:
            third_count = second_count
            third_position = second_position
            second_count = first_count
            second_position = first_position
            first_count = landings[position]
            first_position = position
        elif landings[position] > second_count:
            third_count = second_count
            third_position = second_position
            second_count = landings[position]
            second_position = position
        elif landings[position] > third_count:
            third_count = landings[position]
