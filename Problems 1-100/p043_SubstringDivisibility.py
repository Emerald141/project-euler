...
def solve():
    ...
    result = 0
    for num_string_raw in permutations("1234567890"):
        num_string = ''.join(num_string_raw)
        for index in range(len(primes)):
            if int(num_string[index + 1 : index + 4]) % primes[index] != 0:
                break
        else:
