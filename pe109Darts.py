...
def pe109(cap):
        start = time()
        result = 0
        zones = [x for x in range(1, 21)]
        zones.extend([2 * x for x in range(1, 21)])
        zones.extend([3 * x for x in range(1, 21)])
        zones.extend([25, 50])
        zones.sort()
        ...
        #Two-dart checkouts
        for last in doubles:
                for ind in range(len(zones)):
                        if zones[ind] + last >= cap:
                                result += ind
                                break
                else:
                        result += len(zones)
        ...
        for last in doubles:
                for lowind in range(len(zones)):
                        for highind in range(lowind, len(zones)):
                                if last + zones[lowind] + zones[highind] >= cap:
                                        result += highind - lowind
                                        break
                        else:
                                result += len(zones) - lowind
        peresult(109, result, time() - start)
