...
##there are infinitely many polynomial functions that can model the sequence.
##
...
##linear relationship and predict the next term to be 15 (common difference
##7). Even if we were presented with the first three terms, by the same
...
##be most sensible to assume constancy; that is, for n ≥ 2, OP(1, n) = u1.
##
...
from matrixfns import solve

def solve():
    start = time()
    truecoeffs = [(-1) ** x for x in range(11)]
    trueterms = [None] + [findTerm(truecoeffs, n) for n in range(1, 11)]
    result = 0
    for n in range(1, 11):
        matrix = [ [row ** col for col in range(n)] + [trueterms[row]] \
                   for row in range(1, n+1)]
        falsecoeffs = solve(matrix)
        termnum = 1
        while findTerm(truecoeffs, termnum) == findTerm(falsecoeffs, termnum):
            termnum += 1
        result += int(findTerm(falsecoeffs, termnum))
    peresult(101, result, time() - start)

def findTerm(coeffs, termnum):
    result = 0
    for x in range(len(coeffs)):
        result += coeffs[x] * (termnum ** x)
    return result
