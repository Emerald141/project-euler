...
def solve(cap = 9 * 10 ** 18):
	...
	for b in range(2, max_b + 1):
		...
		if is_prime[b]:
			...
			for length in range(1, len(prime_sixths) + 1):
				indices = [i for i in range(length)]
				while True:
					term = max_a // subproduct(prime_sixths, indices)
					print(b, max_a, indices, subproduct(prime_sixths, indices), term)
					result += ((-1) ** length) * term
					if term != 0 and indices[-1] != len(prime_sixths) - 1:
						indices[-1] += 1
						continue
...
if __name__ == "__main__":
	start = time()
	peresult(634, solve(3 * 10 ** 6), time() - start)
